...
ggplot(hbonds_Eco3R,
  ...
  labs(title = "Number of h-bonds in Eco3R aptamer variants",
       x = "Aptamer version",
       y = "Number of h-bonds",
       fill = "Parent") +
  theme_minimal() +
...
# Flagging the parent bars for easy visualization
score_long_ECA1$is_parent <- score_long_ECA1$aptamer_version == "parent"

## Plotting the four HADDOCK metrics for ECA1 aptamer versions

dev.new()
ggplot(score_long_ECA1,
       aes(x   = value,
           y   = aptamer_version,
           fill = interaction(metric, is_parent))) +    #Colours by the metric and parent is coloured red  
  geom_col() +
  ...
  labs(title = " ECA1 HADDOCK metrics",
       x = "Aptamer version",
       y = " Value") +
  theme_minimal()
